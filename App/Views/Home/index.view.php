<div class="container-container">
    <div id="carouselExampleCaptions" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-indicators">
            <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="0" class="active" aria-current="true" aria-label="Slide 1"></button>
            <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="1" aria-label="Slide 2"></button>
            <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="2" aria-label="Slide 3"></button>
        </div>
        <div class="carousel-inner">
            <div class="carousel-item active">
                <img src="public/images/pas1.png" class="d-block w-100" alt="...">
                <div class="carousel-caption d-none d-md-block">
                    <h5>Letné akcie na obzore!</h5>
                    <p>Ušetrite až 20% pri rezervácii do konca júna.</p>
                </div>
            </div>
            <div class="carousel-item">
                <img src="public/images/pas2.png" class="d-block w-100" alt="...">
                <div class="carousel-caption d-none d-md-block">
                    <h5>Zadarmo sprievodca pri ceste nad 1000€</h5>
                    <p>Rezervujte teraz a objavte kúzla sveta s nami.</p>
                </div>
            </div>
            <div class="carousel-item">
                <img src="public/images/pas3.png" class="d-block w-100" alt="...">
                <div class="carousel-caption d-none d-md-block">
                    <h6>Špeciálne ponuky pre skupinové výlety</h6>
                    <p1>Získajte nezabudnuteľné spomienky s priateľmi alebo rodinou.</p1>
                </div>
            </div>
        </div>
        <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="visually-hidden">Previous</span>
        </button>
        <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="visually-hidden">Next</span>
        </button>
    </div>

    <div class="content">
        <div class="destinations">
            <div class="destination">
                <h3>Paríž</h3>
                <p>Potulky uličkami, návšteva Eiffelovej veže alebo piknik pri Seine. Zažite to všetko a viac!</p>
            </div>
            <div class="destination">
                <h3>Praha</h3>
                <p>Hradčany, Staromestské námestie, Vltava. Poznajte históriu a kultúru Prahy v jednom výlete.</p>
            </div>
        </div>
    </div>

    <div class="container text-center">
        <div class="row row-cols-1 row-cols-md-3 row-cols-lg-6 mt-2 g-3 justify-content-center">
            <div class="col">
                <div class="card">
                    <img src="img/ponukaCiprus.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Ciprus</h5>
                        <button>Pozrieť ponuku</button>
                    </div>
                </div>
            </div>
            <div class="col">
                <div class="card">
                    <img src="public/images/ponukaEgypt.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Egypt</h5>
                        <button>Pozrieť ponuku</button>
                    </div>
                </div>
            </div>
            <div class="col">
                <div class="card">
                    <img src="public/images/ponukaTurecko.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Turecko</h5>
                        <button>Pozrieť ponuku</button>
                    </div>
                </div>
            </div>
            <div class="col">
                <div class="card">
                    <img src="public/images/ponukaBulharsko.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Bulharsko</h5>
                        <button>Pozrieť ponuku</button>
                    </div>
                </div>
            </div>
            <div class="col">
                <div class="card">
                    <img src="public/images/ponukaMexiko.png" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Mexiko</h5>
                        <button>Pozrieť ponuku</button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
